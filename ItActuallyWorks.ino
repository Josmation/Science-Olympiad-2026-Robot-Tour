...
// Ramping
// RAMP_UP_COUNTS   — how many counts to spend accelerating at the start
// RAMP_DOWN_COUNTS — how many counts before the target to start slowing down
// MIN_SPEED        — lowest PWM during ramp up / ramp down (must be enough to move)
...
// =============================================================
//  rampedSpeed — returns the speed the robot should be at given
//  how far it has traveled and how far it has left to go
// =============================================================
int rampedSpeed(long avgCounts, long targetCounts, int topSpeed) {
  // Ramp up phase
  if (avgCounts < RAMP_UP_COUNTS) {
    float t = (float)avgCounts / RAMP_UP_COUNTS;          // 0.0 -> 1.0
    return (int)(MIN_SPEED + t * (topSpeed - MIN_SPEED));  // MIN_SPEED -> topSpeed
  }

  // Ramp down phase
  long countsRemaining = targetCounts - avgCounts;
  if (countsRemaining < RAMP_DOWN_COUNTS) {
    float t = (float)countsRemaining / RAMP_DOWN_COUNTS;   // 1.0 -> 0.0
    return (int)(MIN_SPEED + t * (topSpeed - MIN_SPEED));  // topSpeed -> MIN_SPEED
  }
...
void driveDistance(float meters, int speed) {
  ...
  // Start at MIN_SPEED so the ramp begins from the bottom
